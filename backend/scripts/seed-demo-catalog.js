...
const frontendSeedDir = path.resolve(
    __dirname,
...
const buildImagePaths = (categorySlug, productSlug, editionSlug) => ({
    categoryImageUrl: `/seed-images/categories/${categorySlug}.svg`,
    productImageUrl: `/seed-images/products/${productSlug}.svg`,
    productDetailUrl: `/seed-images/products/${productSlug}-detail.svg`,
    variantImageUrl: `/seed-images/products/${productSlug}-${editionSlug}.svg`,
});
...
async function main() {
    ...
    const categoriesDir = path.join(frontendSeedDir, 'categories');
    const productsDir = path.join(frontendSeedDir, 'products');

    fs.rmSync(frontendSeedDir, { recursive: true, force: true });
    ...
    for (const categoryData of catalog) {
        ...
        const categoryDoc = await Category.create({
            ...
            imageUrl: `/seed-images/categories/${categoryData.slug}.svg`,
            order: categoryData.order,
        });
        summary.categories += 1;

        for (const productData of categoryData.items) {
            const productMainPath = path.join(
                productsDir,
            ...
            const productDoc = await Product.create({
                ...
                imageUrls: [
                    `/seed-images/products/${productData.slug}.svg`,
                    `/seed-images/products/${productData.slug}-detail.svg`,
                ],
            ...
            for (const edition of productData.editions) {
                ...
                const variantDoc = await Variant.create({
                    ...
                    imageUrls: [
                        `/seed-images/products/${productData.slug}-${editionSlug}.svg`,
                        `/seed-images/products/${productData.slug}.svg`,
                    ],
