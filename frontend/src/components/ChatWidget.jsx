...
import { getChatProviderStatus, sendChatMessage } from '@/services'
...
const ChatWidget = () => {
  ...
  const effectiveProvider = selectedProvider || providerState?.provider || 'auto'
    ...
    ;(async () => {
      try {
        ...
        if (response?.success) {
          ...
          setSelectedProvider((current) => current || 'local')
        }
      } catch (_error) {
  ...
  const sendFallbackHttp = async (trimmed) => {
    ...
    try {
      const response = await sendChatMessage({
        message: trimmed,
        history: messages.slice(-MAX_HISTORY),
        provider: effectiveProvider === 'auto' ? undefined : effectiveProvider,
      })
      const reply = response?.reply || response?.data?.reply || response?.answer || ''
      setMessages((current) => {
        const next = [...current]
        if (next[assistantIndexRef.current]) {
          next[assistantIndexRef.current] = {
            ...next[assistantIndexRef.current],
            content: reply || 'Mình chưa trả lời được lúc này. Bạn thử lại sau nhé.',
          }
        }
        return next
      })
    } catch (error) {
      const message = error?.message || 'Hệ thống chat tạm thời chưa sẵn sàng.'
      setMessages((current) => {
        const next = [...current]
        if (next[assistantIndexRef.current]) {
          next[assistantIndexRef.current] = { ...next[assistantIndexRef.current], content: message }
        }
        return next
      })
    } finally {
  ...
  const sendMessage = async (text) => {
    ...
    const shouldStream = effectiveProvider === 'agentic' || effectiveProvider === 'local'
    if (shouldStream) {
      sendViaWebSocket(trimmed)
      return
    }

    await sendFallbackHttp(trimmed)
  }
          ...
          <div className="flex items-center justify-between border-b border-rose-100 px-4 py-3">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {['local', 'agentic', 'auto'].map((provider) => {
