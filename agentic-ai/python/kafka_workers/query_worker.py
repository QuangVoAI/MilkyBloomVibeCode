...
def run_worker():
    ...
    while running:
        ...
        try:
            ...
            def make_stream_callback(sid: str, prod: Producer):
                async def stream_callback(token_chunk: str):
                    ...
                    try:
                        ...
                        # Log nhưng không crash pipeline nếu Kafka produce lỗi
                        console.print(f"[yellow]⚠️ Stream callback error: {e}[/]")
            ...
            # Chỉ cache câu không chứa order_id (tránh PII leak + tránh bypass action)
            if not has_order_id:
                set_cached_answer(
                    question=question,
                    answer=final_answer,
