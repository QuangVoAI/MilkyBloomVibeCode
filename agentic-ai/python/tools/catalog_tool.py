...
def lookup_live_catalog(question: str, context: dict | None = None) -> dict:
    ...
    if remote and isinstance(remote, dict) and not remote.get("success", True):
        return {"found": False, "query": keyword or query, "summary": f"Xin lỗi, mình không kết nối được với dữ liệu cửa hàng ({remote.get('message', 'Lỗi kết nối')}). Vui lòng kiểm tra cấu hình SHOP_API_BASE_URL hoặc trạng thái của backend nhé.", "products": []}
